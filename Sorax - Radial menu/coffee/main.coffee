...
class RadialNav
  constructor: (paper, buttons, icons) ->
    @area = paper
      ...
      .addClass 'radialnav'
    @c = @size / 2 # Center
    @r = @size * 0.25 # Outer radius
    @r2 = @r * 0.35 # Inner radius
  ...
  _hint: (btn) ->
    ...
    hint.select('*').attr startOffset: '56'
    hint
  ...
  _buttonOver: (nav) -> ->
    nav._animateButtonHover @, 0, 1, 100, mina.easeinout
    @[2].removeClass 'hide'

  _buttonOut: (nav) -> ->
    nav._animateButtonHover @, 1, 0, 1000, mina.elastic, (->
  ...
  hide: ->
    ...
    @_animateButtons 1, 0, @animDuration, @animDuration * 8, mina.easeinout
